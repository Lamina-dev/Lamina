...
extern "C" LM_API AdtObj* lmx_computer_algebra_symbol(const char* name) {
    return expr_result_ok(lamina::lsr::sym(name ? name : ""));
}

extern "C" LM_API AdtObj* lmx_computer_algebra_parse(const char* source) {
    return expr_result_ok(lamina::lsr::parse_expr(source ? source : ""));
}

extern "C" LM_API AdtObj* lmx_computer_algebra_pi() {
    return expr_result_ok(lamina::lsr::pi());
}

extern "C" LM_API AdtObj* lmx_computer_algebra_euler_number() {
    return expr_result_ok(lamina::lsr::e());
}

extern "C" LM_API AdtObj* lmx_computer_algebra_golden_ratio() {
    return expr_result_ok(lamina::lsr::phi());
}

extern "C" LM_API ExprObj* lmx_computer_algebra_expression_imaginary_unit() {
    return expr_from_result(lamina::lsr::imaginary_unit());
}

extern "C" LM_API ExprObj* lmx_computer_algebra_expression_integer(const LmInt value) {
...
extern "C" LM_API ExprObj* lmx_computer_algebra_expression_promote_value(const lmx::runtime::Value* value) {
...
extern "C" LM_API ExprObj* lmx_computer_algebra_expression_set(const LmInt count, ...) {
    va_list args;
    va_start(args, count);
    std::vector<lamina::lsr::ExprPtr> values;
    std::string error;
...
extern "C" LM_API ExprObj* lmx_computer_algebra_expression_strip_base_value(ExprObj* value) {
    std::string error;
    const auto* expression = checked_expr(value, error);
    if (!expression) return expression_internal_error(std::move(error));
    lamina::ComputationContext context;
...
extern "C" LM_API ExprObj* lmx_computer_algebra_expression_strip_display_value(ExprObj* value) {
    std::string error;
    const auto* expression = checked_expr(value, error);
    if (!expression) return expression_internal_error(std::move(error));
    lamina::ComputationContext context;
