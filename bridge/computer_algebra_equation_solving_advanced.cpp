    ...
    for (const auto& solution : solutions) {
        ...
        for (const auto& [name, expression] : solution) {
            entries.emplace_back(name, Value(
                new ExprObj(std::make_shared<SymbolicExpr>(expression)),
                ValueKind::Expr));
        }
        result->append(Value(new TableObj(std::move(entries)), ValueKind::Table));
    }
    return result;
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_algebra_factor_multivariate(ExprObj* value) {
    return math_internal::checked_expression_operation("algebra.factor_multivariate", value,
...
extern "C" LM_API AdtObj* lmx_computer_algebra_algebra_factor_transcendental_by_name(
    ...
    try {
        ...
        return result_error(MathErrorCode::InternalError, __func__, 
            std::string("CasError(InternalInvariant in algebra.factor_transcendental: ") +
            exception.what() + ")");
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_polynomial_system_full_by_names(
    ...
    try {
        return result_ok(value_solution_tables(
            lamina::Solver::solve_polynomial_system(values, names)),
            ValueKind::Obj);
    } catch (const std::exception& exception) {
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.polynomial_system: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_parametric_system_by_names(
    ArrayObj* equations, ArrayObj* unknowns, ArrayObj* parameters) {
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.parametric_system: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_groebner_basis_by_names(
    ArrayObj* polynomials, ArrayObj* variables) {
    std::vector<lamina::lsr::ExprPtr> expressions;
    std::vector<std::string> names;
    std::string error;
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.groebner_basis: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_reduced_groebner_basis_by_names(
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.reduced_groebner_basis: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_ideal_membership_by_names(
    ExprObj* polynomial, ArrayObj* basis, ArrayObj* variables) {
    std::string error;
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.ideal_membership: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_elimination_ideal_by_names(
    ArrayObj* basis, ArrayObj* variables, LmInt count) {
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.elimination_ideal: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_inequalities_by_name(
    ArrayObj* expressions, ArrayObj* relations, const char* variable) {
    if (!variable || variable[0] == '\0')
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_rational_inequality_by_name(
    ...
    const char* relation, const char* variable) {
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.rational_inequality: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_inequalities_by_symbol(
    ArrayObj* expressions, ArrayObj* relations, ExprObj* variable) {
    std::string name, error;
    if (!checked_symbol_name(variable, name, error))
        return result_error(MathErrorCode::InvalidArgument, __func__, std::move(error));
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_rational_inequality_by_symbol(
    ExprObj* numerator, ExprObj* denominator,
    const char* relation, ExprObj* variable) {
    std::string name, error;
    if (!checked_symbol_name(variable, name, error))
        return result_error(MathErrorCode::InvalidArgument, __func__, std::move(error));
...
extern "C" LM_API AdtObj* lmx_computer_algebra_equation_solving_parametric_piecewise_by_names(
    ...
    try {
        ...
        for (const auto& item : piecewise.cases) {
            ...
            fields.emplace_back(new ExprObj(item.condition), ValueKind::Expr);
            fields.emplace_back(math_internal::solution_tables(item.solutions), ValueKind::Obj);
            cases->append(Value(new AdtObj(
                "ParametricCase", "ParametricCase", std::move(fields)),
                ValueKind::Obj));
        }
        std::vector<Value> fields;
        fields.emplace_back(cases, ValueKind::Obj);
        return result_ok(new AdtObj(
            "ParametricPiecewise", "ParametricPiecewise", std::move(fields)),
            ValueKind::Obj);
    } catch (const std::exception& exception) {
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("solve.parametric_piecewise: ") + exception.what());
    }
}
