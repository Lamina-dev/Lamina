...
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_argument(ExprObj* value) {
...
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_nth_roots(
    ...
    try {
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("complex.nth_roots: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_quadratic_roots(
    ...
    try {
        std::vector<lamina::lsr::ExprPtr> values;
        for (const auto& root : lamina::solve_complex_quadratic(*av, *bv, *cv)) {
            const auto converted = complex_expression(root);
        ...
        return result_error(MathErrorCode::InvalidArgument, __func__, 
            std::string("complex.quadratic_roots: ") + exception.what());
    }
}
...
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_real_part(ExprObj* value) {
    std::string error; const auto* expression = checked_expr(value, error);
    if (!expression) return result_error(MathErrorCode::InvalidArgument, __func__, std::move(error));
...
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_imag_part(ExprObj* value) {
    std::string error; const auto* expression = checked_expr(value, error);
    if (!expression) return result_error(MathErrorCode::InvalidArgument, __func__, std::move(error));
...
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_conjugate(ExprObj* value) {
    std::string error; const auto* expression = checked_expr(value, error);
    if (!expression) return result_error(MathErrorCode::InvalidArgument, __func__, std::move(error));
    return checked_expr_result(lamina::conjugate_checked(*expression));
}
extern "C" LM_API AdtObj* lmx_computer_algebra_complex_analysis_analytic_continuation_by_symbol(ExprObj* value, ExprObj* variable) {
